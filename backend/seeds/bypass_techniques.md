...
> 本文件只沉淀「怎么躲 WAF 检测」的绕过技巧。攻击原语（读哪个文件、打哪个 sink）
> 由 AI 每轮现生成，不进库。技巧是通用包装器，可施加到任意原语上。

> **标注规范（两个独立维度，写在 `风险` 字段里）**：
> - **来源可信度**：`[官方CVE]`（CVE/GHSA/厂商公告）、`[官方手册]`（DB/语言官方文档）、`[公开绕过]`（社区 writeup/众测报告）、`[无锚点]`（机制真实但无权威出处）
> - **实测状态**：`[已实测]`（对目标 WAF 验证过）、`[未实测]`（机制真实但未对腾讯云 WAF 验证）
>
> 例：`[官方CVE][未实测]` = 来源有 CVE 锚点、但对腾讯云 WAF 的绕过效果尚未验证。二者不混用。
...
- **风险**: 需后端 MySQL；符号集有限；对词法归一化（自动补空白）的语义引擎失效。
...
- **风险**: 需后端 MySQL；仅对按 ASCII 空白(0x20/+)建模的 WAF 有效，语义引擎折叠空白或识别 %0b/%0c 后失效。
...
- **风险**: 仅 MySQL；反引号本身可能被规则盯上；语义引擎解析标识符后失效。
...
- **风险**: 单独使用不构成高质量样本（学习循环会对纯 case 维度降权），应与其他技巧叠加；对大小写不敏感匹配的 WAF 无效。
- **模板**: `uNiOn sElEcT`、`AnD`
...
- **风险**: 仅 MySQL（#）与需要尾字符的 --；语义引擎做注释剥除后失效。
...
- **风险**: 仅 PostgreSQL；同一字符串内不能混用 `'` 与 `$$`；tag 不能以数字开头、不能含 `$`；需避开 WAF 对 `$` 的通用过滤。
...
- **风险**: 仅 MySQL 接受点周空白；对词法归一化的语义引擎失效。
...
- **风险**: 需后端 MySQL 且其词法接受宽字符作分隔符（支持有限、需实测）；对按 Unicode 空白建模的 WAF 无效。
...
- **风险**: 依赖 WAF 实现语言（C 系 strlen 截断才有效）；现代栈对 %00 处理不一致，需实测；对不截断 NUL 的引擎无效。
...
- **风险**: 仅 MySQL；部分 WAF 已支持内联注释解析，处理有缺陷时正可绕。
...
- **风险**: 仅 MySQL（regexp binary/soundex 为 MySQL 方言）；语义引擎可识别运算符。
...
- **风险**: LIMIT OFFSET 需 MySQL/MariaDB/SQLite；SUBSTR FROM FOR 需 MySQL；与既有 from_for_no_comma 部分重叠但覆盖 LIMIT 变体。
...
- **风险**: 对已实现注释剥除的语义引擎无效；注释内容不能改变关键字语义（`/**/` 折叠为空白）。
...
- **风险**: 仅 MySQL；反引号包围函数名，正则对 `version` 可能失配。
...
- **风险**: 需可 GROUP BY 的上下文；HAVING 仅作用于聚合后；FLOOR 载荷为报错原语，需目标回显 SQL 错误；仅 MySQL>=5.0。
...
- **风险**: 用恒等表达式替代 1=1 避开 =；语义引擎做常量折叠后会识别恒真。
...
- **风险**: 仅 MySQL 8.0+；JSON_TABLE 语法较复杂需正确构造列定义；语义引擎若解析 JSON 函数可识别。
...
- **风险**: 对词法归一化（折叠空白）的语义引擎无效；低价值但零成本。
...
- **风险**: 需 MySQL 8.0.19+；列数必须匹配；TABLE 无 WHERE(仅 ORDER BY/LIMIT)；对 AST 语义型 WAF 仍可被解析。
...
- **风险**: WAF 拦 NULL 关键字时用 0/false/char()/算术表达式替代；对类型敏感的语义引擎仍可识别。
...
- **风险**: MySQL 兼容 ODBC 大括号语法；需后端 MySQL，且语义引擎可能已覆盖 {x ...} 转义。
...
- **风险**: 仅 MySQL 8.0+；正则模式需精确匹配避免误判；语义引擎可识别。
...
- **风险**: 仅 PostgreSQL；使用 OPERATOR() 后失去特殊运算符优先级(按标准优先级)；需确认目标 search_path 未屏蔽 pg_catalog。
...
- **风险**: 仅 MySQL；需目标能解析 `9.e` 浮点 token；对词法还原的语义引擎可能失效。
...
- **风险**: 仅 MySQL；GET_LOCK 需同名锁未被占用（同一连接/库内），多请求并发需不同锁名；超时可设。
...
- **风险**: 仅 SQLite；GLOB 区分大小写；`[]` 在 glob 中是字符类语法，构造时注意转义；对 AST 语义型 WAF 仍可识别。
...
- **风险**: 仅 MySQL 8 的 TABLE 语句；盲注逐字符速度慢，字符串与整型列比较发生隐式类型转换需脚本处理。
...
- **风险**: 语义引擎常量折叠后识别恒真；各表达式跨库支持度不一（REGEXP/RLIKE 为 MySQL）。
...
- **风险**: WAF 只拦 EXP/EXTRACTVALUE 时漏过冷门报错函数；floor 在 MySQL8 失效，需用 UUID_TO_BIN/NAME_CONST/JSON_KEYS 等替代。
...
- **风险**: 需后端支持 JSON 函数（MySQL5.7+/PG9.4+）；OWASP CRS 942550 已按 PL1 拦截 JSON 运算符，语义型 WAF 可解析。
...
- **风险**: 需 MySQL 5.7+ 且 sys 模式可用；视图字段与 information_schema 略有差异；仅绕系统表名过滤，不改变其它注入难度。
...
- **风险**: 模式已实证（CTF wafwaf writeup、libinjection issue#26）；需多语句支持；对 AST 型引擎为通用盲区（恶意 token 不进入语法树）。
...
- **风险**: 仅 MySQL≤5.7/MariaDB 修复前(MDEV-27066)；小数点和后随字符需符合触发条件(`( ) . , | & % * ^ /` 之一)。GoSecure 已实击 AWS WAF/CloudFront；OWASP CRS 942560(PL2) 已覆盖。
...
- **风险**: 需 JSON 能力的 DB(MySQL5.7+/PG9.4+)且业务处理 JSON；OWASP CRS 942550(2022-12 起)已按 PL1 拦截，语义型 WAF 可解析；模板内双引号需注意 URL 编码不被截断。
...
- **风险**: [公开绕过][未实测] d0znpp 公开绕过 libinjection 系 WAF；对维护完整括号栈的 AST 重建型引擎（如雷池）效果未知，且 payload 在无匹配开括号的 MySQL 上下文中本身是语法错误，需结合外层括号/多语句执行环境。
...
- **风险**: 仅 MySQL <8.0；仅限 LIMIT 之后的注入位置；载荷为报错/时间原语，需目标回显错误或可测时延。
...
- **风险**: 依赖 WAF 表名提取正则的具体形态(Coze Studio 实例)；MySQL 接受 `FROM (tbl)` 语法；对语义/AST WAF 无效。
...
- **风险**: 已实证（libinjection issue #26，直接报告 "not sqli"）；需后端支持多语句（mysqli multi_query/连接串 allowMultiQueries）。
...
- **风险**: 已实证 libinjection 系；对穷举内置函数 token 的引擎效果随函数表更新而衰减。ST_LatFromGeoHash 报错型在 MySQL 5.7+ 有效。
...
- **风险**: 仅对 Upper+Contains 实现有效(Coze Studio 实例)；对正则词边界或 AST 解析型 WAF 无效；与通用反引号标识符技巧易混淆，本质是检测架构盲区。
...
- **风险**: MySQL 5.6+(weight_string 可用)；level 子句为非常规参数形态，新版本语义引擎可能已覆盖；来源为雷池官方解密文章，属历史实证。
...
- **风险**: 需应用回显数据库错误信息(verbose error)；仅 PostgreSQL；部分 WAF 会匹配 cast/::int 组合，需再套大小写/注释混淆。
...
- **风险**: 仅 PostgreSQL CHR；位运算需注意运算符优先级(建议加括号)；属数值混淆层，对语义引擎效果有限。
...
- **风险**: MySQL 双上下文行为已实证（0x 在语法早期被当作 BINARY 字面量转换，绕过普通带引号字符串解析路径）；MySQL 8.0 仍保有该双语义。
...
- **风险**: [官方手册][未实测] MySQL 隐式类型转换行为有官方手册依据；作为'绕过语义引擎'的效果取决于引擎是否做严格类型折叠，尚未实测。
...
- **风险**: 仅 MySQL；需目标列支持 BINARY 转换；语义引擎若还原大小写敏感性可识别。
...
- **风险**: [官方手册][未实测] 语法有 MySQL 官方手册依据（charset introducer）；对 libinjection/雷池类引擎的绕过效果未实测，可作 fuzz 起点。
...
- **风险**: 仅目标连接/存储字符集为 GBK 类双字节；现代 PHP 用 mysql_real_escape_string/mysqli 参数化已免疫；宽字节绕过在目标明确 GBK 时高可靠。
...
- **风险**: 仅 MySQL 5.6+ 且 InnoDB 引擎；mysql.innodb_table_stats 字段与 information_schema 有差异（需用 database_name 过滤）；访问权限需足够。
...
- **风险**: 仅 MySQL 且目标设置了 NO_BACKSLASH_ESCAPES（现代框架少用，需确认目标配置）；对参数化查询无效。
...
- **风险**: [官方CVE][未实测] mod_security/libinjection issue #1601 实证阿拉伯文误归一化导致漏检；攻击模板对目标引擎的效果未实测。
...
- **风险**: 仅 Oracle 10g+；分隔符需选择字面量中不出现的字符；对识别 q 引号形态的语义引擎可被解析。
...
- **风险**: 仅 Oracle 且需 CTXSYS 组件可用（标准安装默认有）；需错误回显。
...
- **风险**: 仅 Oracle；依赖错误回显；不同 Oracle 版本对 XMLType 报错形态有差异。
...
- **风险**: 仅 Oracle；USERENV 上下文参数名需正确（CURRENT_USER/DB_NAME/DB_UNIQUE_NAME/HOST/CLIENT_IP/SESSION_USER 等）。
...
- **风险**: 仅 Oracle；需数据库允许出站（11g+ 默认受限 ACL，需 DBMS_NETWORK_ACL_ADMIN 放行）；属盲注/带外原语，需攻击者可控接收端。
...
- **风险**: 仅 Oracle；需应用回显数据库错误信息；包名随版本有差异需实测。
...
- **风险**: 仅 MSSQL；堆叠查询需后端 ADO 支持多语句；xp_cmdshell 默认关闭需开启；属高危执行原语需授权环境。
...
- **风险**: 仅 MSSQL；需错误回显；`CONVERT` 常被 WAF 盯（可再套大小写/注释）。
...
- **风险**: [无锚点][未实测] 仅 MSSQL；盲注需可区分响应差异；无权威绕过出处，真实性依赖具体场景，未实测。
...
- **风险**: 仅 MSSQL；需相应权限（BULK 需 bulkadmin）；现代默认受限；属高影响原语需授权环境。
...
- **风险**: 仅 MSSQL；时间盲注需稳定测时；`WAITFOR` 在部分上下文需 `;` 分隔。
...
- **风险**: 实证（某池语义 WAF v9.3.5 + MySQL 5.7.26）；仅对按通用 SQL 语义建模、不区分方言的引擎有效。
...
- **风险**: 腾讯门神 WAF 众测实证；仅 MySQL；依赖词法器对数字别名与 hex 大小写的具体处理。
...
- **风险**: 腾讯门神 WAF 众测实证；仅 MySQL；依赖引擎对每种方言函数的文法覆盖程度。
...
- **风险**: 实证（某池语义 WAF v9.3.5，user() 可替换、database() 失效，需具体请求具体分析）；冷门函数可能被语义引擎更新覆盖。
...
- **风险**: 实证（某池语义 WAF v9.3.5 + Tomcat 9）；依赖后端取首个 Content-Type、WAF 取末个的差异。
...
- **风险**: bash 内建(非交互 shell 默认可用)；需 shell 支持 exec -a，sh/dash 不支持。
...
- **风险**: 注入点需保留换行（HTTP 中用 %0a 编码）；仅对按行/单 token 建模的 WAF 有效，做跨行还原的语义引擎可检测。
...
- **风险**: POSIX 支持 ${x:-c}；bash 独有部分展开形态；对做参数展开求值的语义引擎可还原。
...
- **风险**: `\x` 是 GNU 扩展、八进制 `\ooo` 才可移植；Picus/F5 2020 已针对 rev+printf 组合补过签名（存在被更新检测的风险）；对能模拟并求值命令替换的语义引擎可被还原。
...
- **风险**: 依赖具体环境变量值（PWD 长度、SHLVL、`$-` 内容等）跨环境不稳定；其中 `${var:0:1}` 取环境变量字符与既有 cmd_env_substring 类别部分重叠，新颖点在于 `$-`/`$#`/`$SHLVL` 这类特殊参数作字符源。
...
- **风险**: `~` 仅在词首、赋值 = 后、路径冒号后展开，引号内不展开；展开结果不再次展开（`~root/$(id)` 的命令替换由无引号后缀触发）；路径后缀被无引号拼进 shell 即构成注入。
...
- **风险**: 依赖 tr 可用；平移产出的字符有限、需按需构造；对能模拟字符集转换的语义引擎可被还原。
- **模板**: `$(tr '!-}' '\"-~'<<<[)`（产生反斜杠）
...
- **风险**: `$_` 为 bash 扩展非 POSIX；需上一命令已执行且 `$_` 被更新（纯赋值语句不更新 `$_`）；设置 debug trap 会破坏其值。
...
- **风险**: 仅 Windows CMD 的 %Var:~a,b% 子串语法；需目标为 cmd.exe。
...
- **风险**: 对剥离注释的引擎失效；注释仅能加在 shell 允许的位置。
...
- **风险**: bash<<<$(...) 或 heredoc 喂命令，依赖 bash/heredoc；base64 关键字本身可能被拦。
...
- **风险**: 用无害命令稀释 token 序列；对做命令意图分类的语义引擎可能仍识别出后段 cat。
...
- **风险**: PowerShell 反引号/-f/-EncodedCommand/字符串拼接；需 PowerShell 环境。
...
- **风险**: cat <(命令) 进程替换执行命令，bash-only；dash 不支持 <()。
...
- **风险**: Windows CMD 用 ^ 转义下一字符，c^a^t 解析后还原；需 cmd.exe 环境。
...
- **风险**: PowerShell 直调 .NET API 替代命令词；需 PowerShell 环境；对建模 .NET 调用的检测器失效。
...
- **风险**: BASH_ENV 仅 bash 非交互；env 包装器常见。
...
- **风险**: 需支持 @P 的 bash(>=5.0)；GNU 官方 2026-06 安全通告证实对"命令安全评估层"有效；对完整模拟参数展开并求值的语义引擎可能被还原。
...
- **风险**: 需目标环境有 awk/gawk/mawk；GTFOBins 标准 GTFOBin，广泛可用。
...
- **风险**: coreutils 常见；cat 仍以整行字符串出现，仅对首词式检测有效。
...
- **风险**: find 常见；-exec 处仍含 cat 字符串，若检测扫描整条命令行含 cat 会被拦；GTFOBins 标准项。
...
- **风险**: 需 git 且输出触发分页(长输出或 `-p` 且 stdout 为 tty)；短输出/管道环境不触发。
...
- **风险**: 需 perl 解释器；GTFOBins 标准项；反引号内命令名若被"全局字符串过滤"仍会被拦。
...
- **风险**: 需对应解释器；各为 GTFOBins 标准项；若检测整行匹配 cat 字符串仍被拦。
...
- **风险**: GNU sed 专有(BSD sed 无 e)；文件需存在且可读；GTFOBins 标准项。
...
- **风险**: 需 xargs；cat 仍以字符串出现在参数流，若检测整行含目标命令名仍会被拦。
...
- **风险**: 注入点需能带换行/heredoc；语义引擎若还原 `$0`/`$SHELL` 实际值则失效；`. /dev/stdin` 依赖 /dev 可用。
...
- **风险**: 仅 zsh 环境可用；`${(e)var}` 再展开的攻击原语部分未在本检索中实证；对支持 zsh 展开并求值的语义引擎可被还原。
...
- **风险**: 对做完整 AST 求值/字符串还原的语义检测（如 PowerPeeler）可被还原；AST 结构具 Invoke-Obfuscation 特征、易被行为检测标记。
...
- **风险**: 对解析 AST 并求值字符串的检测器可被还原；纯关键字黑名单可绕过，语义/行为引擎仍可捕获。
...
- **风险**: 别名在行解析时展开，单行内定义并调用会失败；必须让 alias 定义先于调用行被读取。
...
- **风险**: 需 bash；检测扫描整行含 cat 字面仍会被拦；Unix.SE 有 `${!a[$(id>&2)]}` 下标名注入变体。
...
- **风险**: 需 bash（`exec 3<...`）；cat 命令名仍出现，只规避文件路径过滤。
...
- **风险**: 需 bash 且后续真正以该命令名触发；仅投毒时静态不可见真实载体，适合绕过按命令名黑名单的引擎。
...
- **风险**: [官方手册][未实测] command -v/type 内建行为有官方手册依据；对只识别'执行位置'的语义抽取更有效，未实测。
...
- **风险**: 仅交互 shell 生效；非交互需 `set -H` 或 `shopt -s histexpand`，适用性窄。
- **模板**: `id`、`!!`、`sudo !!`、`cd !$`
...
- **风险**: 需 bash 且目标端口可连；被建模为重定向的引擎易漏。
...
- **风险**: [官方CVE][未实测] CVE-2014-6271（Shellshock）实证函数体后续内容被执行；新版 bash 仅收 BASH_FUNC_* 命名空间且需触发函数，未实测。
...
- **风险**: 需交互 bash 渲染提示符；在 shell 启动文件/环境注入场景有效。
...
- **风险**: 需 bash 且在 shell 上下文内；静态不易被识别为命令执行。
...
- **风险**: 需 bash 且 /dev/fd 可用；若引擎已对 `<()` 建模则减效。
...
- **风险**: 仅 zsh 且 EQUALS 开启；对 bash 建模的引擎是盲区。
- **模板**: `=sh -c 'id'`、`echo =cat`
...
- **风险**: 仅 fish；对 bash/posix 建模引擎是盲区。
- **模板**: `echo (id)`、`(id)`
...
- **风险**: [官方手册][未实测] zsh 手册有 e/N 限定符语法；e 限定符执行依赖 zsh 扩展，未实测。
...
- **风险**: [官方手册][未实测] bash 手册有 ${!prefix*} 前缀枚举；多跳间接展开对引擎建模负担高，未实测。
...
- **风险**: 针对放行 cid:/id 协议且不编码属性值的清洗配置；触发依赖同名 id clobber 后的属性合并。
...
- **风险**: CSP 白名单域常带 JSONP 端点，借白名单域执行任意回调；需白名单域存在 JSONP 端点。
...
- **风险**: 需后端存在 <style> CSS 上下文；WAF 若按 HTML 上下文整体解析则失效。
...
- **风险**: 需 CSP/过滤放行图片标签；仅数据外带（非直接执行）；对阻断 img 的过滤失效。
...
- **风险**: blob:/data: URI + 动态 import() 构造可执行 JS 上下文；需现代浏览器 ESM。
...
- **风险**: 需前端存在 innerHTML/eval 等 sink；WAF 只查传输层、DOM 拼接在客户端完成；对前端运行时检测失效。
...
- **风险**: 浏览器容错解析一致可靠；仅对启发式/正则 WAF 有效。
...
- **风险**: 冷门事件处理器（onpointerenter/onauxclick 等）WAF 黑名单常不全；部分依赖现代浏览器。
...
- **风险**: AngularJS/Vue/React 框架上下文执行 JS；需目标使用对应框架且存在表达式注入点。
...
- **风险**: Vue v-html/React dangerouslySetInnerHTML 等把属性值当代码渲染；需目标框架误用。
...
- **风险**: 绕过基于再序列化的过滤层（对纯 WAF 检测层是全新形态：本库 mXSS@query 0% 的墙可能被这类**形态本身**打开——WAF 没建过模）。
...
- **风险**: 需 meta refresh 不被过滤；部分浏览器对 javascript: 于 refresh 的支持有差异，需实测。
...
- **风险**: onpointerrawupdate 需指针经过元素；onscrollend 需真实滚动；onanimationstart 需 CSS 动画。均需一定用户交互但无需点击具体控件。
...
- **风险**: 需用户交互（点击后输入），Chrome/Firefox/Edge 支持；要求元素可聚焦。
...
- **风险**: 需能注入 popover 属性与事件；Chrome/Edge 支持；点击触发，非零交互。
...
- **风险**: 页面校验 origin 用 includes/前缀匹配时可用 attacker.com 前缀欺骗。
...
- **风险**: [官方CVE][未实测] CVE-2024-8372（AngularJS ngSrcset）实证绕过域名限制；data:image/svg+xml 在 srcset 的加载执行依赖浏览器，Chrome 支持、部分浏览器未实测。
...
- **风险**: 仅对未剥离命名空间前缀的清洗器生效；修复做法是先取冒号后段（localName）再比对。
...
- **风险**: 仅对不阻断 SMIL 动画元素、只做静态属性校验的清洗器/网关有效；需 SVG 以 image/svg+xml 呈现且渲染期支持动画。
...
- **风险**: 需 sanitizer 允许 SVG 动画元素及 values 属性；服务端静态校验对 URI-list 语义无效。
...
- **风险**: 需 XML/SVG 响应上下文；WAF 若剥离命名空间前缀(localName 比对)则失效。
...
- **风险**: 需 SVG 上下文；对剥离 xlink:href 或校验 javascript: 协议的 WAF 失效。
...
- **风险**: [官方CVE][未实测] CVE-2026-48761/GHSA-hhg7-c65m-h7ff（Symfony）、GHSA-97xv-3v84-h358（SiYuan）实证；poster 的 javascript: 各浏览器行为不一，未实测。
...
- **风险**: HTML 属性解析容忍注释，但仅对按字面匹配属性名的 WAF 有效；语义引擎还原后失效。
...
- **风险**: 对做 JS AST 分析/字符串还原的检测器失效；纯关键字黑名单可绕过。
...
- **风险**: 正则字面量 .source 返回模式文本，运行时拼出目标字符串；payload 中不出现目标字符串。
...
- **风险**: 需 JS 表达式上下文；对建模反引号模板语法的引擎失效。
...
- **风险**: 针对 Chrome 内置 Sanitizer API 特定版本；原理可外推到所有按字符串匹配剥离命名空间属性的清洗器。
...
- **风险**: Chrome 已修复；该条主要为"URL 解析器差异可被利用"的通用方法论，具体载荷需针对目标解析器差分测试。
...
- **风险**: 从 alert 逐级轮换到 OOB 外带，证明执行而不依赖 alert；成功判定不绑定 alert。
...
- **风险**: WAF 与浏览器解析 HTML 的差异（命名空间、原始文本模式、标签容错）导致 WAF 看到无害/浏览器解析出 XSS。
...
- **风险**: 取决于后端框架取首键/末键/合并的语义（PHP/Rails 取末键，ASP.NET 合并）；WAF 以首个键做判定时生效。
...
- **风险**: 主要影响 XML 解析路径或对 CDATA 段内容放行的服务端清洗器；内联到 HTML 上下文时生效。
...
- **风险**: 需自定义元素被放行（非默认配置）；DOMPurify 3.2.4 后部分缓解；允许自定义标签的自研清洗器同样适用。
...
- **风险**: 需 SAFE_FOR_TEMPLATES=true 且允许自定义元素（配置相关），默认配置不受影响。
...
- **风险**: [官方手册][未实测] MDN 明确 setHTMLUnsafe 不做清洗且可含 DSD；仅在应用误用 unsafe API 或 SSR 直拼时成立，属上下文错配，未实测。
...
- **风险**: Chrome/Edge 111+ 经 HTML 解析（SSR/正常 parse）创建 DSD 时内容进入活动 shadow root；若只经 innerHTML 赋值则 template 保持惰性，需改事件/URL gadget；Firefox 支持不完全。
...
- **风险**: 依赖浏览器重解析；对做注释内容递归检查的 sanitizer 失效。
...
- **风险**: HTML/MathML 命名空间切换，浏览器重解析时 <style> 内容被当 HTML 解析，逃过只查顶层标签的规则。
...
- **风险**: 依赖浏览器重解析造成标签错位；对建模重解析行为的 sanitizer 失效。
...
- **风险**: 需 SVG+foreignObject+iframe 组合重解析；对剥离 SVG 或 iframe 的过滤失效。
...
- **风险**: 需浏览器重解析触发；template 惰性内容在部分 sink 不激活。
...
- **风险**: 影响按解析树静态判断祖先命名空间的清洗器；DOMPurify 3.1.3 修复；自研 HTML 清洗或按 DOM 树审计场景仍可复用；模板为示意，原 PoC 见 mizu.re。
...
- **风险**: 针对对同一字符串连续 sanitize 两次的应用层加固；解析轮次被消耗完即失效；模板为示意，原 PoC 见 mizu.re。
...
- **风险**: 需清洗后的 HTML 被放入 noscript/xmp/noembed/noframes/iframe/textarea 等原始文本上下文；DOMPurify 3.1.3~3.3.1 / 2.5.3~2.5.8 受影响。
...
- **风险**: 需 sanitizer 开启 XML/XHTML media type 且输出被嵌入 HTML 上下文；3.0.11 后已修复，对存量版本与自研 XML 清洗器仍有效。
...
- **风险**: 需页面存在相对路径的带 nonce script 且 base-uri 未设；Chrome/Firefox 一致。CSP 配 base-uri 'none' 或 'self' 即失效。
...
- **风险**: 需能触发受信脚本的反射/sink，非通用；nonce 复用或泄漏时成立。属最常被低估的 CSP 盲点之一。
...
- **风险**: 依赖目标受信脚本的具体实现（存在可滥用 sink/gadget 才成立），属应用相关，未对特定目标实证；机制层面成立。
...
- **风险**: 需目标站存在同源可回显/上传内容到 iframe 的路径；属 spec 设计缺口而非浏览器漏洞，严格 CSP 也可能被绕。
...
- **风险**: 需 SAFE_FOR_TEMPLATES + RETURN_DOM 类输出 + 下游调用 normalize()；3.4.8 后修复。
...
- **风险**: 需目标应用注册了修改属性允许列表的全局钩子；为应用集成层漏洞而非纯 payload。
...
- **风险**: 取决于安全库是否以 document.implementation 为 fallback 条件；对多数现代库需另找可 clobber 的依赖。
...
- **风险**: 腾讯门神 WAF 众测实证；依赖引擎解码层数与浏览器/后端解码层数的具体错位。
...
- **风险**: 腾讯门神 WAF 众测实证；依赖引擎 HTML 解析器与浏览器的差异。
...
- **风险**: 腾讯门神 WAF 众测实证；依赖引擎 JS 语法版本覆盖。
...
- **风险**: 需 Apache/PHP 映射这些扩展名为脚本；对按完整扩展名列表白名单的 WAF 失效。
- **模板**: `.pht`、`.phtml`、`.phar`、`.pgif`、`.php5`

### upload:extension:case_ext — 大小写扩展名

- **原理**: shell.PHP 大小写扩展名绕过只匹配小写 .php 的 WAF。
- **风险**: Windows/IIS 大小写不敏感；Linux Apache 大小写敏感时 .PHP 可能不执行，需实测。
...
- **风险**: 对取末扩展名做黑名单的应用失效；不同服务器解析行为不一。
...
- **风险**: 对规范化文件名并拒绝尾点的应用失效；Windows 专属。
...
- **风险**: 针对未过滤反斜杠的 WAF（CRS <4.6.0 可绕过）；需要后端在保存时保留反斜杠后的扩展名或服务端按特定规则还原；对严格规范化文件名并拒绝 `\` 的 WAF 失效。
...
- **风险**: 需后端支持 RFC5987 filename* 解码（如 Java）；对不解码 filename* 的后端失效。
...
- **风险**: 需后端保存时保留原始文件名且不做超长处理（直接拒绝或 hash 重命名即失效）；截断位置依赖 OS/应用实现需实测；对强制限定扩展名白名单并重命名的应用无效。
...
- **风险**: 针对按单行匹配头部规则的 WAF；后端需把换行当空白容忍；部分实现可能把换行当作部件边界导致拆分失败。
...
- **风险**: 需 Windows/IIS 且 NTFS；对 Linux 后端无效；对剥离 ::$DATA 的 WAF 失效。
...
- **风险**: 需后端 move_uploaded_file 未做路径规范化或直接拼接文件名；对执行 basename()/realpath 白名单校验的应用无效；部分版本对 `..` 直接拒绝。
...
- **风险**: 需后端自动解码 RFC2047 文件名（Java Commons-FileUpload）；对不解码的后端失效。
...
- **风险**: 主要用于绕过按显示扩展名校验的过滤器或诱导人工审核；浏览器/现代系统会黑名单化 U+202E；对基于文件真实内容或 magic bytes 的检测无效。
...
- **风险**: 仅 IIS 在分号截断文件名；对 IIS7+ 或非 IIS 后端无效。
...
- **风险**: 解析器行为差异大，需逐一对目标 WAF/后端实测；部分严格解析器会直接报错拒绝；非通用。
...
- **风险**: 需后端在安全校验之后才做 NFKC/ASCII 规范化；DNN FileUploader（CVE-2025-52488，NTLM 凭据窃取）实证；对规范化顺序正确的实现失效。
...
- **风险**: multipart 里声明 image/jpeg 实际传 PHP 代码；对做内容检测（魔数/解码）的 WAF 失效。
...
- **风险**: exiftool 把 XSS 写进图片元数据，应用渲染元数据时触发存储型 XSS。
...
- **风险**: 需存在可控 script src 的 XSS 注入点与可执行 uploads 目录；对开启 nosniff + Content-Disposition: attachment 的上传服务失效。
...
- **风险**: 需应用存在 phar:// 触发点（file_exists 等接收用户可控路径）与可用 POP 链；phar.readonly=Off 才能在本地生成；纯作为上传载荷绕过过滤器是成熟手法。
...
- **风险**: 需应用后续以 ZIP 方式解压该文件；对执行图像重编码（重新解码再编码）的防御失效；polyglot 结构可被 typebleed 检出。
...
- **风险**: 需浏览器以内联方式访问上传文件且无 nosniff/attachment 头；Grav（GHSA-xc64-vh46-vph6）与 Camaleon（GHSL-2024-184）实证；SVG 重编码/严格 MIME 白名单可防御。
...
- **风险**: 需 PHP-FPM 且允许 .user.ini；对扫描 .user.ini 内容的 WAF 失效。
...
- **风险**: 针对依赖 ZIP Method 字段的恶意软件/WebShell 扫描器；对不信任头字段、按内容重解压的检测失效；github bombadil-systems/zombie-zip 提供 PoC。
...
- **风险**: 需 Apache 且 AllowOverride 允许 FileInfo；对禁止 .htaccess 上传或忽略其配置的环境失效。
...
- **风险**: 需 Apache AllowOverride 允许 FileInfo 且目录可执行；PHP>=7 无短标签时需 base64 编码；PolarCTF/长城杯 SQLUP 等实战使用；对 php_value 系指令或 filter/resource 关键字有规则的 WAF 失效。
...
- **风险**: 需 PHP-FPM 且 user_ini.filename 未被禁；对扫描 .user.ini 内容的 WAF 失效。
...
- **风险**: IIS 上传 web.config 改 handler 映射让 .jpg 走 ASP.NET 执行；需 IIS 环境。
...
- **风险**: 需服务器配置把含 .php 中间扩展名的文件映射到 PHP handler；Laravel-Mediable（CVE-2026-49972，未认证 RCE）与 Spatie Media Library（CVE-2026-48557）实证；移除遗留 AddHandler 即免疫。
...
- **风险**: PHP 专属的 boundary 宽松解析；若 WAF 采用与 PHP 相同截断规则则失效；需实测目标 PHP 版本。
...
- **风险**: 需后端支持 RFC 2231 续延而 WAF 不支持；长亭公开复现在某云 WAF 上绕过 SQLi 检测（2024）；对已实现续延解析的 WAF 失效。
...
- **风险**: 针对使用正则按字面匹配扩展名的 WAF；对先 trim 再校验的应用失效；Java 服务端场景。
...
- **风险**: 针对未处理非打印字符的 WAF（CRS <4.6.0 实证可绕过）；新版 CRS 已拦截；后端需容忍该字符。
...
- **风险**: 需后端解析器对重复参数取法与 WAF 不一致；@hapi/content <6.0.2（CVE-2026-44974）实证；对 strict RFC7578 实现的过滤器可能无效。
...
- **风险**: [无锚点][未实测] 需后端递归解析嵌套 multipart 而 WAF 只展平；Falcon 默认不递归、Go 1.20.6 前有 boundary 解析错位；嵌套格式已被 RFC 弃用，通用效果未实测。
...
- **风险**: PHP 专属；依赖 PHP 与 WAF 对该字段名解析差异；需实测 PHP 版本对方括号计数的具体行为。
...
- **风险**: PHP 专属宽松解析；对严格拒绝畸形请求的 WAF 反而会放行但应用可能拒绝；需实测 PHP 版本。
...
- **风险**: PHP 专属且依赖 max_file_uploads 为默认 20；需要请求体内同时包含大量文件部件，可能被请求体大小限制拦截。
...
- **风险**: 主要针对 Nginx+PHP 场景；需 WAF 把该部分误判为文件上传而跳过检测；PHP 严格匹配 `filename=` 完整序列。
...
- **风险**: 针对严格按 RFC 用双引号的 WAF；对同时支持单引号的 WAF 失效；PHP/Flask/Java 行为需实测。
...
- **风险**: Rack <2.2.23/3.1.21/3.2.6（CVE-2026-26961）；需上游组件取首个 boundary；对规范化的边缘代理已修补。
...
- **风险**: python-multipart <0.0.30（CVE-2026-53537）实证；需后端做 RFC2231 解码而 WAF 不做；0.0.30+ 已忽略扩展参数。
...
- **风险**: 需 Apache 启用 Includes(mod_include) 且上传目录可执行；#exec 常被 IncludesNOEXEC 禁用，此时只剩文件读取；WAF 对 .shtml 上传与 SSI 指令语法的检测覆盖普遍缺失，但需实测目标 Apache 是否解析 .shtml。
...
- **风险**: 需应用对上传的 XML/SVG 做 XSLT 变换或接受用户可控样式表；libxslt/Saxon 未限制外部访问时文件读/SSRF 成立；php:function 需 PHP XSL 扩展启用；与 svg_stored_xss 的纯前端路径不同。
...
- **风险**: [公开绕过][未实测] GNOME Epiphany #2233 实证 http(s) 页面加载含 XSL 的 SVG 可读文件；通用上传服务器场景未实测。
...
- **风险**: [无锚点][未实测] NTFS 8.3 短名是文件系统硬机制（fsutil 可禁）；短名枚举+上传在 Windows 实战有案例，但针对具体 WAF 的绕过效果未实测。
...
- **风险**: 需 Nginx 未加 try_files $uri =404 且 PHP cgi.fix_pathinfo=On；现代 Nginx 默认已修复；修复=try_files+cgi.fix_pathinfo=0+security.limit_extensions；对 URL 层拦截 /.php 后缀的 WAF 失效。
...
- **风险**: 仅 Windows Server 2003 / IIS 6.0（IIS 7+ 已修复）；需攻击者能控制上传目录名或覆盖到 .asp 结尾目录；WAF 若校验整个 URL 路径含 .asp 目录段则失效；环境较老仅遗留系统适用。
...
- **风险**: 依赖 PatternLayout/配置文件里消息 lookup 可用(2.15 前默认开)；上下文键不存在时走默认值。
...
- **风险**: 需要 date lookup 的格式参数被当作字面量输出(SimpleDateFormat 引号语义)；纯字符串过滤型 WAF 有效。
...
- **风险**: date lookup 格式字符拼接关键字；仅对纯字符串过滤型 WAF 有效。
...
- **风险**: 仅对把 URL/域名当连续子串匹配的 WAF 有效；date 输出随机化会破坏需精确 host 的检测。
- **模板**: `${jndi:ldap://127.0.0.1#${date:'MM-dd-yyyy'}}`

### log4j2:lookup:deep_nesting — 深嵌套递归

- **原理**: `substitute()` 无限递归，WAF 单层匹配在多层级嵌套下失手。
- **风险**: 依赖 Log4j 递归插值；2.16.0+ 移除消息 lookup 后失效；对建模递归展开的引擎可还原。
...
- **风险**: 需目标运行于容器/K8s；信息泄露型，Cloudflare 在野观测已确认 docker/k8s 用于外带。
...
- **风险**: U+0131 经 ${upper:ı}→I 拼出 jndi；仅对不还原 upper/lower 的 WAF 有效。
...
- **风险**: 仅对纯字符串过滤型 WAF 有效；语义引擎还原默认值展开后失效。
...
- **风险**: 定向读 pod 环境变量里的密钥，配合嵌套混淆仍可读出。
...
- **风险**: 两层嵌套只有 Log4j 递归插值能还原；对拒绝任意 `${}` 递归展开的保守语义引擎可能反而触发告警。
...
- **风险**: 需目标 JNDI 服务支持对应协议(rmi/dns/iiop)；对覆盖全部协议的 WAF 失效。
...
- **风险**: URI.getHost() 取 # 前值，LDAP 连接全主机名（绕 2.15 部分检查）。
...
- **风险**: 信息泄露型(非 RCE)；需日志内容可被攻击者注入；对覆盖非 jndi 前缀的 WAF 失效。
...
- **风险**: IPv6 映射 IPv4 形式表示回环地址，绕点分 IPv4 字面量黑名单。
...
- **风险**: k8s/docker lookup 需对应插件在 classpath；容器环境外 key 仍缺失、默认值照样触发。
...
- **风险**: 信息泄露型，均可锚定官方文档；java: 不可用于 Android。
...
- **风险**: 仅对不还原 lower/upper 的 WAF 有效；语义引擎还原后失效。
...
- **风险**: 需 MainMapLookup.setMainArguments 已调用且日志内容可被攻击者注入；信息外带型，Cloudflare 在野观测已确认。
...
- **风险**: 默认值机制需 lookup 键缺失；仅对未覆盖 map: 前缀归一化的 WAF 有效。
...
- **风险**: ${${what:ever:-j}...} 非存在 lookup 求值默认值拼关键字。
...
- **风险**: 非默认端口运行 JNDI 服务，绕对标准 ldap 端口建模的 WAF。
...
- **风险**: 需应用使用 log4j-spring-boot 且日志配置带 ${} 求值；回显的是目标 Spring 属性而非直接 RCE，属探测/泄露型。
...
- **风险**: 默认值机制需属性缺失；仅对未覆盖 sys: 归一化的 WAF 有效。
...
- **风险**: 须运行在 Servlet 容器(web lookup 由 tomcat/websphere 等提供)；缺键时走默认值。
...
- **风险**: 要求 PatternLayout 含 `$${ctx:...}`/`%X` 且攻击者可控 MDC；2.16.0 后移除消息 lookup，失效。
...
- **风险**: 技术事实来自 LOG4J2-2109/2.17.0 设计文档；作为防御面判定而非新绕过——2.16.0+ 消息 lookup 已不存在。
...
- **风险**: CVE-2021-45105，2.0~2.16.0 受影响；已封死消息 lookup 的 2.17+ 不适用。
...
- **风险**: 对不做 JSON 深度解码的原包正则型 WAF 有效；语义引擎解一轮 unicode 即失效。
...
- **风险**: 前提是攻击者已具备改配置文件的权限（配置投毒/供应链），非远程无鉴权可达；source 为 CVE 数据库。
